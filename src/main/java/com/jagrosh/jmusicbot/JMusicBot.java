...
public class JMusicBot 
{
    ...
    private static void startBot()
    {
        ...
        // Discord requires E2EE (the DAVE protocol) on voice connections. JDA's built-in
        // passthrough session is rejected with close code 4017, so real libdave natives
        // must be present or no audio will ever play.
        try
        {
        ...
        {
            prompt.alert(Prompt.Level.ERROR, "JMusicBot", "Could not load the libdave natives, which Discord now "
                    + "requires for voice connections: " + t
                    + "\nEnsure the moe.kyokobot.libdave natives artifact for this platform is on the classpath.");
            System.exit(1);
        }
